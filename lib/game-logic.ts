...
export function calculateEffectiveWeights(
  ...
  for (const stat of TRAINING_STATS) {
    // Base calculation: athlete weight - opponent stat
    let effective = athleteWeights[stat] - opponentStats[stat];
...
export function generateAdvice(
  ...
  if (score >= threshold * 0.9 && score < threshold) {
    ...
    if (bestGainStat) {
      tips.push({
        message: `So close! Add a small boost to **${getStatLabel(bestGainStat.stat)}**.`,
        priority: 1,
      });
    }
  }
  ...
  for (const { stat, weight, slider } of highWeightStats) {
    if (weight > 0.5 && slider < 5) {
      tips.push({
        message: `We needed a bit more **${getStatLabel(stat)}** to match them.`,
        priority: 2,
      });
    }
  }
  ...
  for (const { stat, weight, slider } of lowWeightStats) {
    if (weight < 0.3 && slider > 7) {
      tips.push({
        message: `We trained **${getStatLabel(stat)}** a lot, but it didn't help much here.`,
        priority: 3,
      });
    }
  }
